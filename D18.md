...
Attributes:
    arg_constraints (dict): Dictionary specifying the argument constraints.
    support (Constraint): Constraint specifying the support of the distribution.
    has_rsample (bool): Flag indicating whether the distribution has a reparameterized sampling method.

Methods:
    expand(batch_shape, _instance=None): Expand the distribution to a given batch shape.
    rsample(sample_shape=()): Generate random samples from the distribution.
    log_prob(value): Compute the log probability of a given value.
    entropy(): Compute the entropy of the distribution.
...
Properties:
    ...
    _natural_params: Natural parameters of the distribution.
    _log_normalizer: Log normalizer of the distribution.
"""
```
