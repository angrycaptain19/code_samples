...
class AfterShipSensor(SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        """Register callbacks."""
        self.async_on_remove(self._force_update)
    ...
    async def async_update(self, **kwargs: Any) -> None:
        ...
        for track in trackings["trackings"]:
            status = track["tag"].lower()
            name = track.get("title", track["tracking_number"])
            last_checkpoint = track["checkpoints"][-1] if track["checkpoints"] else ""
            status_counts[status] = status_counts.get(status, 0) + 1
            parsed_trackings.append(
                {
                    "name": name,
        ...
        self._state = len(parsed_trackings) - status_counts.get("delivered", 0)
```
