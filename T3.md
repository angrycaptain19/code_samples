...
from autogpt.app.main import get_user_feedback, UserFeedback

@pytest.mark.parametrize(
    "console_input, expected_feedback, expected_input, expected_cycles",
    [
        ("", UserFeedback.TEXT, "", None),
        ("yes", UserFeedback.TEXT, "yes", None),
        ("AUTHORIZE", UserFeedback.AUTHORIZE, "", None),
        ("authorize -5", UserFeedback.AUTHORIZE, "", 5),
        ("exit", UserFeedback.EXIT, "", None),
    ]
)
def test_get_user_feedback(
    console_input, expected_feedback, expected_input, expected_cycles, config, ai_config
):
    user_inputs = [console_input]
    with patch("autogpt.app.main.clean_input", side_effect=user_inputs):
