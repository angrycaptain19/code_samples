...
class ExampleAuthProvider(AuthProvider):
    ...
    def async_validate_login(self, username: str, password: str) -> None:
        """Validate a username and password."""
        user = next((usr for usr in self.config["users"] if hmac.compare_digest(
            username.encode("utf-8"), usr["username"].encode("utf-8")
        )), None)
    ...
    async def async_get_or_create_credentials(
        ...
        """Get credentials based on the flow result."""
        username = flow_result["username"]
        credential = next((cred for cred in await self.async_credentials() if cred.data["username"] == username), None)
    ...
    async def async_user_meta_for_credentials(
        ...
        """
        username = credentials.data["username"]
        user = next((usr for usr in self.config["users"] if usr["username"] == username), None)
