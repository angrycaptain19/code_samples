...
def get_user_feedback(
    ...
    while True:
        console_input = clean_input(
            config, "Waiting for your response..." if config.chat_messages_enabled else f"{Fore.MAGENTA}Input:{Style.RESET_ALL}"
        )
        ...
        if console_input.lower().strip() == config.authorise_key:
            ...
            try:
                return UserFeedback.AUTHORIZE, "", abs(int(console_input.split(" ")[1]))
            except ValueError:
                logger.warn(
                    f"Invalid input format. "
                    f"Please enter '{config.authorise_key} -N'"
            ...
            return UserFeedback.TEXT, console_input, None
The simplified code removes the need for the user_feedback, user_input, and new_cycles_remaining variables by directly returning the values when the conditions are met. It also simplifies the loop by using a while True loop and returning the values immediately when a condition is met.
```
